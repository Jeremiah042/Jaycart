...
import { useOutSideClick } from "@/src/Hook";
    ...
    >
      ...
      >
        <div className="flex items-center justify-between gap x">
          ...
          <button
            onClick={onClose}
            className="hover:text-shop-green-500 hoverEffect"
          >
            {" "}
            <X />{" "}
          </button>
        ...
        <div className="flex flex-col space-y-3.5 font-semibold tracking-wide">
          {headerData?.map((item) => (
            <Link
              href={item?.href}
              key={item?.title}
              className={`hover:text-shop-emerald-500 hoverEffect ${pathname === item?.href && "text-white"}`}
            >
              {item?.title}
            </Link>
          ))}
